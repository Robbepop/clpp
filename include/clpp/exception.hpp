...
namespace cl {
	namespace error {
		...
		public:
			...
			virtual auto code() const -> RetCode = 0;
		};

//		class DeviceNotFound : public AnyError {
//		public:
//			using type = typename DeviceNotFound;
//			DeviceNotFound(): AnyError{"device not found"}{}
//			RetCode code() const override { return RetCode::deviceNotFound; }
//		};

		#define EXCEPTION(ExceptionName,Code,Msg) \
			class ExceptionName final : public AnyError {\
			public:\
				using type = ExceptionName;\
				ExceptionName():\
					AnyError{Msg}\
				{}\
				RetCode code() const override noexcept {\
					return Code;\
				}\
			};
